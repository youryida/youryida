...
var view = {
  ...
  getCellProperties(index, col) {
    if (index in gPrefView) return gLockProps[gPrefView[index].lockCol];
  ...
  cycleHeader(col) {
    ...
    if (col.id == gSortedColumn) {
      ...
      gPrefArray.reverse();
      if (gPrefView != gPrefArray) gPrefView.reverse();
      if (index >= 0) index = gPrefView.length - index - 1;
    } else {
      var pref = null;
      if (index >= 0) pref = gPrefView[index];
      ...
      if (gPrefView != gPrefArray) gPrefView.sort(gSortFunction);
      gSortedColumn = col.id;
      if (pref) index = getViewIndexOfPref(pref);
    }
...
function getViewIndexOfPref(pref) {
  ...
  while (index > low) {
    ...
    if (mid == pref) return index;
    if (gSortFunction(mid, pref) < 0) low = index;
    else high = index;
    index = (low + high) >> 1;
  }
  return -1;
...
function getNearestViewIndexOfPref(pref) {
  ...
  var index = (low + high) >> 1;
  while (index > low) {
    if (gSortFunction(gPrefView[index], pref) < 0) low = index;
    else high = index;
    index = (low + high) >> 1;
  }
  return high;
...
function getNearestIndexOfPref(pref) {
  ...
  var index = (low + high) >> 1;
  while (index > low) {
    if (gSortFunction(gPrefArray[index], pref) < 0) low = index;
    else high = index;
    index = (low + high) >> 1;
  }
  return high;
...
var gPrefListener = {
  observe(subject, topic, prefName) {
    ...
    if (updateArray) {
      ...
      if (addedRow) view.treebox.rowCountChanged(newIndex, 1);
      ...
      if (selectedIndex >= 0) {
        ...
        if (selectedIndex == newIndex) view.treebox.ensureRowIsVisible(selectedIndex);
      }
    }
  }
};
...
function fetchPref(prefName, prefIndex) {
  ...
  if (gPrefBranch.prefIsLocked(prefName)) pref.lockCol = PREF_IS_LOCKED;
  else if (gPrefBranch.prefHasUserValue(prefName)) pref.lockCol = PREF_IS_MODIFIED;
...
function onConfigLoad() {
  ...
  if (showWarning) document.getElementById("warningButton").focus();
  else ShowPrefs();
}
...
function ShowPrefs() {
  ...
  if (descending.item(0)) {
    ...
    if (ascending.item(0)) gSortedColumn = ascending[0].id;
...
function ModifySelected() {
  if (view.selection.currentIndex >= 0) ModifyPref(gPrefView[view.selection.currentIndex]);
}
...
function NewPref(type) {
  ...
  if (
    ...
    var pref;
    if (result.value in gPrefHash) pref = gPrefHash[result.value];
    else
      pref = { prefCol: result.value, lockCol: PREF_IS_DEFAULT_VALUE, typeCol: type, valueCol: "" };
    if (ModifyPref(pref)) setTimeout(gotoPref, 0, result.value);
  }
}
...
function ModifyPref(entry) {
  if (entry.lockCol == PREF_IS_LOCKED) return false;
  ...
  if (entry.typeCol == nsIPrefBranch.PREF_BOOL) {
    ...
    if (
      !entry.valueCol &&
      !Services.prompt.select(window, title, entry.prefCol, 2, [false, true], check)
    )
      return false;
    ...
    var dummy = { value: 0 };
    if (!Services.prompt.prompt(window, title, entry.prefCol, result, null, dummy)) return false;
